  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-[20px] shadow-sm h-16 flex justify-between items-center px-6 md:px-10 max-w-full mx-auto transition-all duration-300">
      ...
      <div className="flex items-center gap-10">
        <Link to="/" onClick={scrollToTop} className="text-xl font-extrabold tracking-tight text-[#2b3437] font-headline cursor-pointer">
      ...
      <div className="flex items-center gap-3">
        {!isDashboard ? (
          <div className="hidden md:flex items-center gap-3">
            <Link to="/login"><button className="px-4 py-1.5 text-[#586064] font-bold hover:text-[#0c56d0] transition-colors text-sm font-body">Sign In</button></Link>
            <Link to="/register"><button className="bg-[#0c56d0] hover:bg-[#004aba] text-[#f8f7ff] px-5 py-2 rounded-lg font-bold transition-all shadow-sm text-sm font-headline">Get Started</button></Link>
          </div>
        ) : (
          <div className="flex items-center gap-5 relative">
            <div className="relative">
              <button onClick={() => setNotifOpen(!notifOpen)} className="material-symbols-outlined text-[#abb3b7] cursor-pointer hover:text-[#0c56d0] transition-colors mt-1.5">notifications</button>
              {unreadCount > 0 && <span className="absolute top-1 right-0 w-4 h-4 bg-red-600 text-white text-[8px] font-black rounded-full flex items-center justify-center border-2 border-white pointer-events-none">{unreadCount}</span>}
              <AnimatePresence>
                {notifOpen && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 mt-4 w-80 bg-white rounded-[2rem] shadow-2xl border border-slate-100 z-[200] overflow-hidden">
                    ...
                    <div className="max-h-80 overflow-y-auto custom-scrollbar">
                      {notifications.length === 0 ? (
                        <div className="py-12 px-6 text-center"><span className="material-symbols-outlined text-slate-200 text-4xl">notifications_off</span><p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest mt-3">All caught up</p></div>
                      ) : (
                        notifications.map((n) => (
                          <div key={n.id} onClick={() => markAsRead(n.id)} className={`p-5 border-b border-slate-50 cursor-pointer transition-colors hover:bg-slate-50 text-left ${!n.isRead ? 'bg-blue-50/30' : ''}`}>
                            <div className="flex justify-between items-start mb-1"><p className={`text-xs font-bold ${n.type === 'warning' ? 'text-red-600' : 'text-[#0c56d0]'}`}>{n.title}</p>{!n.isRead && <div className="w-1.5 h-1.5 bg-[#0c56d0] rounded-full mt-1" />}</div>
            ...
            <div className="flex items-center gap-3 border-l border-[#abb3b7]/20 pl-5">
              <div className="w-8 h-8 rounded-full bg-[#0c56d0]/10 border border-[#0c56d0]/20 flex items-center justify-center text-[#0c56d0] font-bold text-xs">KA</div>
              {/* FIXED LOGOUT ACTION */}
              <button onClick={handleLogout} className="text-xs font-bold text-[#9f403d] hover:underline uppercase tracking-wider">Logout</button>
            </div>
          </div>
        )}
      ...
      <AnimatePresence>
        {isOpen && !isDashboard && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-16 left-0 w-full bg-white border-b border-[#abb3b7]/15 p-6 flex flex-col gap-5 md:hidden shadow-xl">
            {!isAuthPage && (
              <>
